"""
Spotlight Queue Builder Module

This module handles the construction and management of Snapchat Spotlight post queues.
It manages the scheduling, organization, and validation of spotlight posts for
multiple clients, handling both regular and TikTok content.

Features:
--------
1. Queue Management:
   - Spotlight post organization
   - Client grouping
...
router = APIRouter(prefix="/api/spot-queue")
...
class SpotQueueBuilder:
    """
    Spotlight Queue Builder for Snapchat Posts.
    
    Handles the construction and management of spotlight post queues for
    scheduled Snapchat spotlight publishing.
    
    Attributes:
        client: MongoDB client connection
        upload_db: Upload database instance
        queue_db: Queue database instance
        spotlights: Spotlights collection
        spot_queue: Spotlight queue collection
        MORNING_POST_TIME: Base time for morning posts (UTC)
        POST_START_MINUTES: Starting minute offset
        
    Notes:
        - Manages spotlight posts
        - Handles time blocks
    ...
    def __init__(self):
        ...
        self.upload_db = self.client['UploadDB']
        self.queue_db = self.client['QueueDB']
        self.spotlights = self.upload_db['Spotlights']
        self.spot_queue = self.queue_db['SpotQueue']
        self.MORNING_POST_TIME = 16
        self.POST_START_MINUTES = 0

    async def build_daily_queue(self, queue_date: datetime = None) -> dict:
        """
        Build the daily queue for all clients.
        
        Args:
            queue_date (datetime): Optional date for queue building
        ...
        Notes:
            ...
            - Handles both regular and TikTok sessions
            - Validates content
            - Manages scheduling
        """
        if not queue_date:
        ...
        await self.spot_queue.delete_many({"queue_date": queue_date.strftime('%Y-%m-%d')})
        ...
        print(f"Total documents in collection: {await self.spotlights.count_documents({})}")
        ...
        async for doc in cursor:
            try:
                ...
                # Get the client's snap_ID at the document level
                client_snap_id = doc.get('snap_ID')
                print(f"\n=== Processing client {client_count}: {client_id} (snap_ID: {client_snap_id}) ===")
                ...
                # Try to queue files - pass the client_snap_id
                print(f"Calling _prepare_client_queue for {client_id}")
                client_queue = await self._prepare_client_queue(client_id, all_sessions, client_snap_id)
        ...
        print("REACHED SAVE OPERATION - ABOUT TO SAVE TO DATABASE")

        # Save queue to database (always save) - using upsert like queue_builder.py
        try:
            result = await self.spot_queue.update_one(
                {"queue_date": daily_queue["queue_date"]},
                {"$set": daily_queue},
                upsert=True
            )
            print(f"Queue {'updated' if result.modified_count else 'created'} with {daily_queue['total_posts']} posts")
        except Exception as e:
    ...
    async def _prepare_client_queue(self, client_id: str, client_sessions: list[dict], client_snap_id: str) -> list[dict]:
        """
        Prepare a queue for a client by selecting unqueued files from their sessions.
        
        Args:
            client_id (str): Client identifier
            client_sessions (list[dict]): List of client session data
            client_snap_id (str): Client's Snapchat ID
            
        Returns:
            list[dict]: List of prepared post data
        ...
        Notes:
            ...
            - Manages captions
        """
        print(f"  Preparing queue for client {client_id} with snap_ID {client_snap_id}")
        queued_files = []
        ...
        for session in client_sessions:
            ...
            for file in files:
                ...
                # Use the client's snap_ID for the post
                post_data = {
                    'file_name': file_name,
                    'cdn_url': file.get('cdn_url'),
                    'file_type': file_type,
                    'snap_id': client_snap_id,  # Use the client's snap_ID here
    ...
    async def mark_files_as_queued(self, client_id: str, queued_files: list):
        ...
        Args:
            client_id (str): Client identifier
            queued_files (list): List of files to mark as queued
            
        Notes:
            - Updates regular files
            - Updates TikTok files
            - Tracks queue time
            - Handles errors
        """
        try:
...
async def build_queue():
    ...
    try:
        ...
        queue = await builder.build_daily_queue()
